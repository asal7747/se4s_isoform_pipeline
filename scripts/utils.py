...
try:
    from scripts import cluster_cells as cc  # when running via package/tests
except ImportError:
    pass  # when running `python scripts/utils.py` from repo root
...
def load_short_read_datasets(
    data_dir: str = "outputs/anndata",
) -> dict[str, sc.AnnData]:
    """
...
def short_read_adata_to_df(adata: sc.AnnData, make_unique: bool = True) -> pd.DataFrame:
    """
...
def qc_and_filter(
    ...
    # print highly expressed genes (only when interactive)
    ...
    qc = sc.pp.calculate_qc_metrics(
        adata,
        qc_vars=(),  # <- empty collection, not None (fixes TypeError in tiny tests)
        percent_top=None,  # important for tiny test datasets
        log1p=False,
        inplace=False,
    )
    cell_qc_dataframe = qc[0]

    # optionally print/show highest expressed genes
    if plot_qc:
    ...
    # only show plots when requested
    show_plots = bool(plot_qc)
    ...
    # make a violin plot of some of the computed quality measures (only if interactive)
    if show_plots:
        sc.pl.violin(
            adata,
...
def load_and_qc_h5ad(
    ...
    """
    Minimal wrapper around qc_and_filter for a single h5ad file.

    - Reads the specific input h5ad.
    - Runs qc_and_filter.
    - Writes <stem>_qc.h5ad into out_dir.
    ...
    # Ensure qc_and_filter uses a stable name for the output file
    adata.uns["dataset_name"] = in_path.stem

    qc_adata = qc_and_filter(
        adata,
    ...
    # qc_and_filter writes <dataset_name>_qc.h5ad; expect that here
    expected = out_path / f"{in_path.stem}_qc.h5ad"
    if expected.is_file():
        return str(expected)

    # Fallback if dataset_name was changed inside qc_adata
    ds_name = qc_adata.uns.get("dataset_name", in_path.stem)
    alt = out_path / f"{ds_name}_qc.h5ad"
    if alt.is_file():
        return str(alt)

    raise FileNotFoundError(
        f"Expected QC output not found in {out_path} "
        f"(looked for {expected.name} and {alt.name})"
    )
...
def main():
    ...
    try:
        out_qc = load_and_qc_h5ad(
            in_h5ad, out_dir, min_counts=min_counts, min_genes=min_genes
        )
        print(f"QC and filtering completed successfully -> {out_qc}")
