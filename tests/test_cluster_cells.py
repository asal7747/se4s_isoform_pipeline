...
from scripts import cluster_cells, run_cluster_cells  # noqa: E402
...
def larger_qc_h5ad(tmp_path):
    ...
    Uses realistic count-like data that will pass highly variable gene call.
    """
    np.random.seed(42)
    ...
    # Housekeeping genes (genes 350-500): similar across all cells
    for i in range(350, 500):
        X[:, i] = np.random.poisson(lam=50, size=100) + 10
...
def test_custom_resolution_affects_clustering(tmp_path, monkeypatch):
    """Test that different --resolution values produce different
    cluster counts."""
...
def test_without_force_recluster_uses_existing_leiden(tmp_path,
                                                      monkeypatch,
                                                      capsys):
    """Test that existing leiden clustering is used when
    --force-recluster is not provided."""
    # Create h5ad with existing leiden column
    ...
    assert "Using existing leiden clustering from input file" in captured.out
...
def test_save_figures_creates_tsne_file(tmp_path, larger_qc_h5ad, monkeypatch):
    ...
    assert tsne_file.is_file(), f"t-SNE plot file should be created at "
    f"{tsne_file}"
...
def test_all_plots_generated_together(tmp_path, larger_qc_h5ad, monkeypatch):
    ...
    # Use fewer PCs appropriate for test data
    argv = ["cluster_cells.py", str(larger_qc_h5ad), str(out_csv),
            "--pca", "--tsne", "--umap", "--save-figures", "--output-dir",
            str(output_dir), "--n-pcs", "10", "--n-neighbors", "5"]
    monkeypatch.setattr(sys, "argv", argv)
    run_cluster_cells.main()

    # Check that all three plot types were created
    # (with default color_by=leiden)
    pca_file = output_dir / "pca_leiden.png"
    tsne_file = output_dir / "tsne_leiden.png"
    umap_file = output_dir / "umap_leiden.png"
...
def test_normalize_column_name_returns_none_if_not_found(tmp_path,
                                                         monkeypatch):
    """Test that normalize_column_name returns None
...
def test_existing_leiden_column_case_variations(tmp_path, monkeypatch, capsys):
    """Test that existing leiden clustering is detected
    ...
    assert "Using existing leiden clustering from input file" in captured1.out
    ...
    assert "Using existing Leiden clustering from input file" in captured2.out
